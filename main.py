...
<div class=pos><b>Posiciones ({len(data['pos'])}/5) - Auto: <span id=autoPos>...</span> | Lógica: RSI&lt;32 + Precio&gt;EMA20 + BTC&gt;-1.5% + Trailing</b><br><br><div id=pos>{pos_html}</div><br><small>MILLONARIO 100% - Manual respeta filtros | Usa BTC FORCE para forzar | /auto on/off /status</small></div>
<script>
async function L(){{
 try{{
  ...
  btn.style.color='#000';
  txt.innerText=st.auto_buy?'Millonario activo':'Solo manual con filtro';
  ...
  for(let s in d){{
   ...
   let filtTxt=d[s].filt? (d[s].rsi<32?'✅ MILLONARIO':'✅ Filtros OK') : '⏸️ BLOQUEADO';
...
def buy(sym):
    ...
    is_force="FORCE" in sym.upper()
    sym=sym.replace("FORCE","").strip()
    if len(data['pos'])<5 and not any(p['sym']==sym for p in data['pos']):
        rsi,price,ema20,btc_t=AN(sym)
        filt=price>ema20 and btc_t>-1.5
        if (rsi<32 and filt) or is_force:
    ...
    if "message" in d:
        ...
        if "FORCE" in txt:
            ...
            if sym in data["coins"] and len(data["pos"])<5 and not any(p['sym']==sym for p in data["pos"]):
                ...
                tg(chat,f"⚠️ {sym} FORZADO ${price:.2f} RSI {rsi:.1f} - SIN FILTRO MILLONARIO")
            return {"ok":True}

        # MILLONARIO - respeta lógica
        if txt in data["coins"]:
            if len(data["pos"])>=5:
                tg(chat,f"⚠️ Ya tienes 5 posiciones")
            elif any(p['sym']==txt for p in data["pos"]):
                tg(chat,f"⚠️ Ya tienes {txt}")
            else:
                ...
                if rsi<32 and filt:
                    ...
                    tg(chat,f"✅ {txt} MILLONARIO ${price:.2f} RSI {rsi:.1f} EMA ${ema20:.1f} BTC {btc_t:.2f}% - 3 FILTROS OK")
                else:
                    tg(chat,f"⏸️ {txt} BLOQUEADO RSI {rsi:.1f} EMA ${ema20:.1f} BTC {btc_t:.2f}% - NO ES MILLONARIO\nUsa {txt} FORCE para forzar")
        ...
        if "/AUTO" in txt:
            ...
            save()
            tg(chat,f"{'✅ AUTO ON - Millonario activo' if data['auto_buy'] else '⏸️ AUTO OFF - Solo manual con filtro millonario'}")
        if "/STATUS" in txt or "/ESTADO" in txt:
            total,val=totals()
            tg(chat,f"🤖 V28.1 MILLONARIO\nAuto: {'ON ✅' if data.get('auto_buy',True) else 'OFF ⏸️'}\nSaldo ${data['b']:.2f} Total ${total:.2f}\nPos {len(data['pos'])}/5\nLógica: RSI<32 + EMA20 + BTC>-1.5%")
        ...
        if "/START" in txt: tg(chat,"V28.1 MILLONARIO 100%\nLógica: RSI<32 + EMA20 + BTC>-1.5%\nManual respeta filtro\nUsa BTC FORCE para forzar\n/auto on/off /status /reporte\n/dashboard")
        save()
    return {"ok":True}
        ...
        try:
            for sym in data["coins"]:
                ...
                if data.get('auto_buy',True) and rsi<32 and price>ema20 and btc_t>-1.5 and len(data["pos"])<5 and not any(p['sym']==sym for p in data["pos"]):
                    data["pos"].append({"sym":sym,"monto":50,"gan":0,"precio_entry":price,"max_price":price}); data["b"]-=50; data["trades_hoy"]+=1; save()
                    for u in data["alert_users"]: tg(u,f"🤖 AUTO MILLONARIO {sym} RSI {rsi:.1f} ${price:.2f} EMA OK BTC {btc_t:.2f}%")
                for p in data["pos"][:]:
                    if p['sym']==sym:
