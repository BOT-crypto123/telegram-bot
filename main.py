...
default_data = {
 ...
 "pct_mes": 0.0,
 "ganadas": 0, "salidas": 0,
 "tp": 0.5, "sl_pct": -1.5, "rsi_compra": 35, "rsi_venta": 70,
 "filtro_ema": "ON", "max_entradas": 3,
 "auto": True, "pos": [], "historial": [], "capital_history": [],
...
def get_prices_data():
    ...
    for sym in SYMS:
        try:
            ...
            print(sym,e)
            out[sym.replace("/USDT","")] = {"price":0,"rsi":50,"limite":35,"p_ema_ok":False,"ok":False,"sug":"Error","motivo":str(e)}
    return out
...
@app.route("/dashboard")
def dashboard():
    # Esto arregla tu 404, busca el archivo en la misma carpeta
    if os.path.exists("dashboard.html"):
        return send_from_directory(".","dashboard.html")
    else:
        return "No existe dashboard.html en GitHub, súbelo",404

@app.route("/chart/<sym>")
def chart_page(sym):
    return f"<h1>{sym} Grafica - Proximamente</h1><a href='/dashboard'>Volver</a>"

@app.route("/api/prices")
def api_prices():
    return jsonify(get_prices_data())

@app.route("/api/state")
def api_state():
    bola = data["capital_actual"]/data["max_entradas"]
    tp_neto = data["tp"] - (FEE*2*100)
    winrate = (data["ganadas"]/data["salidas"]*100) if data["salidas"]>0 else 0
    # Calcular gan por pos
    prices = get_prices_data()
    for p in data["pos"]:
        pr = prices.get(p["sym"],{}).get("price",p["entry"])
        p["ahora"]=pr
        gan_b = (pr-p["entry"])/p["entry"]*100
        gan_n = gan_b - (FEE*2*100)
        p["gan_neta_pct"]=gan_n
        p["gan_neta_mxn"]= p["monto"]*gan_n/100 * data["usd_mxn"]
        p["debe_vender"] = gan_n >= (data["tp"]-FEE*2*100) or gan_n <= data["sl_pct"]
    return jsonify({
        "meta_mxn": data["usd_mxn"]*500,
        "gan_acum": data["gan_acum_total"],
        "gan_acum_mxn": data["gan_acum_mxn"],
...
def api_config():
    ...
    if "toggle_coin" in j:
        s=j["toggle_coin"]
        data["coins_activas"][s]=not data["coins_activas"].get(s,True)
...
def sell_sym(sym):
    ...
    for p in data["pos"]:
        if p["sym"]==sym:
            ...
            gan_b=(pr-p["entry"])/p["entry"]*100
            gan_n=gan_b - FEE*2*100
            gan_mxn=p["monto"]*gan_n/100*data["usd_mxn"]
            data["capital_actual"]+=p["monto"]*gan_n/100
            data["gan_acum_total"]+=p["monto"]*gan_n/100
            data["gan_acum_mxn"]+=gan_mxn
            data["salidas"]+=1
            ...
            # === AVISO A TELEGRAM DE GANANCIA ===
            for uid in data["alert_users"]:
                tg(uid, f"{'🟢 GANANCIA' if gan_mxn>0 else '🔴 PERDIDA'} {sym} {gan_n:.2f}% = ${gan_mxn:.2f} MXN\nCapital: ${data['capital_actual']:.2f}")
            data["pos"].remove(p)
            save()
            break
    return jsonify(ok=True)
        ...
        try:
            if data["auto"]:
                ...
                for p in list(data["pos"]):
                    ...
                    gan_b=(pr-p["entry"])/p["entry"]*100
                    gan_n=gan_b - FEE*2*100
                    rsi_v = prices.get(p["sym"],{}).get("rsi",100)
                    ...
                    if debe:
                        ...
                        data["capital_history"].append({"t":int(time.time()*1000),"cap":data["capital_actual"]})
                        for uid in data["alert_users"]:
                            tg(uid, f"{'🟢 GANANCIA' if gan_mxn>0 else '🔴 PERDIDA'} {p['sym']} {gan_n:.2f}% = ${gan_mxn:.2f} MXN\nCapital: ${data['capital_actual']:.2f}")
                        data["pos"].remove(p)
                        save()
                # COMPRAS
                if len(data["pos"])<data["max_entradas"]:
                    for sym,info in prices.items():
                        if info["ok"] and data["coins_activas"].get(sym,True) and not any(x["sym"]==sym for x in data["pos"]):
                            ...
                            save()
                            for uid in data["alert_users"]:
                                tg(uid, f"🔵 COMPRA {sym} ${info['price']:.2f} RSI {info['rsi']}")
                            break
