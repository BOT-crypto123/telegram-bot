...
def get_monto(nivel):
    return 500 if nivel==1 else 750
...
<div class=top>
<div><b>{sym}</b> <span class=live>● VIVO 1M</span><br><small id=entryinfo style=color:#ffcc00>Cargando entrada...</small></div>
...
<div id=chart></div>
<div style=padding:8px;text-align:center;font-size:10px;color:#888'>🟩 VERDE = Tu entrada | 🟦 AZUL = TP +1.5% Venta rápida | Se mueve cada 3 seg - NETO ya menos 0.6%</div>
<script>
const SYM="{sym}";
...
async function init(){{
  try{{
    ...
    if(p){{entry=p.precio_entry; monto=p.monto;
      ...
      document.getElementById('entryinfo').innerText='SIN POSICION - Grafica mercado vivo 1M';
    }}
    ...
    let url=`https://api.binance.com/api/v3/klines?symbol=${{binSym}}&interval=1m&limit=200`;
    let kl=await fetch(url).then(r=>r.json());
  ...
  }}catch(e){{ document.getElementById('entryinfo').innerText='Error cargando, recarga'; }}
}}
init();
...
def h(m):
    ...
    if "RESET5K CONFIRMAR" in txt:
        ...
        data["com_total"]=0
        save()
        bot.send_message(uid,"✅ V38.4 FINAL 6POS $5000 - LINEAS + BOLA + NETO + NO SE CAE", reply_markup=markup)
        return
    elif "RESET5K" in txt:
        bot.send_message(uid,"⚠️ Escribe RESET5K CONFIRMAR para borrar todo", reply_markup=markup)
        return
    if any(k in txt for k in ["DASH","BALANCE","SALDO","START","HOLA"]):
        tot,flot=totals()
        bot.send_message(uid,f"V38.4 FINAL 6POS 🔥\n💰 Total NETO ${tot:.2f}\nSaldo ${data['b']:.2f}\nFlot NETO {flot:+.2f}$\nPos {len(data['pos'])}/6\nAUTO {'ON' if data.get('auto') else 'OFF'}\nHist NETO ${data.get('gan_total',0):+.2f}\nCom ${data.get('com_total',0):.2f}\nBola $500/$750 SI\n$2000 libres SI\nLineas SI\nhttps://telegram-bot-cijp.onrender.com", reply_markup=markup)
    elif txt in ALL_COINS:
        if len(data["pos"])>=MAX_POS:
            bot.send_message(uid,f"❌ Lleno {MAX_POS}/{MAX_POS} - Te deja $2000 libres para bola a propósito", reply_markup=markup)
        else:
            nivel=sum(1 for x in data["pos"] if x["sym"]==txt)+1
            if nivel>2:
                bot.send_message(uid,f"❌ {txt} ya tiene 2/2 bola N1+N2", reply_markup=markup)
            else:
                monto=get_monto(nivel)
                if data["b"]<monto:
                    bot.send_message(uid,f"❌ Saldo ${data['b']:.2f} necesita ${monto} para N{nivel}", reply_markup=markup)
                else:
                    pr=P(txt)
                    if pr==0:
                        bot.send_message(uid,"❌ Sin precio, intenta 5 seg", reply_markup=markup)
                    else:
                        data["pos"].append({"sym":txt,"monto":monto,"precio_entry":pr,"gan":0,"max_price":pr,"nivel":nivel})
                        data["b"]-=monto
                        save()
                        bot.send_message(uid,f"✅ BOLA N{nivel} {txt} ${pr:.2f} x ${monto}\n🟩 Entrada ${pr:.2f}\n🟦 TP ${pr*1.015:.2f} Neto +0.9%\nCom ${monto*COMISION:.2f}\nLibres ${data['b']:.2f}\nhttps://telegram-bot-cijp.onrender.com/chart/{txt}", reply_markup=markup)
    elif "AUTO ON" in txt:
        data["auto"]=True
        save()
        bot.send_message(uid,"AUTO V38.4 6POS ON 🔥 Bola $500/$750 + Lineas + $2000 libres", reply_markup=markup)
    elif "AUTO OFF" in txt:
        data["auto"]=False
        save()
        bot.send_message(uid,"AUTO OFF - Pausado", reply_markup=markup)
            ...
            if data.get("auto"):
                for p in list(data["pos"]):
                    ...
                    if pct <= -15:
                        ...
                        gan=bruto-com_total
                        data["b"]+=p["monto"]+gan
                        data["gan_total"]=data.get("gan_total",0)+gan
                        data["com_total"]=data.get("com_total",0)+com_total
                        data["pos"].remove(p)
                        save()
                        for uid in data["alert_users"]:
                            try:
                                bot.send_message(uid,f"🛑 STOP NETO {p['sym']} N{p.get('nivel',1)} {pct:.1f}% Neto ${gan:.2f} Com ${com_total:.2f}")
                            except:
                                pass
                        ...
                        for uid in data["alert_users"]:
                            try:
                                bot.send_message(uid,f"💰 VENTA NETA {p['sym']} N{p.get('nivel',1)} {pct:.1f}% Neto ${gan:.2f} (Bruto ${bruto:.2f} -Com ${com_total:.2f})")
                            except:
                                pass
                for sym in ALL_COINS:
                    ...
                    if count>=2 or len(data["pos"])>=MAX_POS:
                        continue
                    nivel=count+1
                    monto=get_monto(nivel)
                    if data["b"]<monto:
                        continue
                    should=False
                    if count==0 and rsi<45:
                        should=True
                    elif count==1:
                        entries=[x["precio_entry"] for x in data["pos"] if x["sym"]==sym]
                        if len(entries)>0 and pr < sum(entries)/len(entries)*0.97:
                            should=True
                    if should:
                        data["pos"].append({"sym":sym,"monto":monto,"precio_entry":pr,"gan":0,"max_price":pr,"nivel":nivel})
                        data["b"]-=monto
                        save()
                        for uid in data["alert_users"]:
                            try:
                                bot.send_message(uid,f"🔥 BOLA N{nivel} {sym} ${pr:.2f} x ${monto} RSI {rsi:.0f} Libres ${data['b']:.2f}\nhttps://telegram-bot-cijp.onrender.com/chart/{sym}")
                            except:
                                pass
            time.sleep(20)
        except:
        ...
        try:
            ...
            if now_mex.hour==22 and now_mex.minute==0:
                ...
                if last_report_date!=today_str:
                    tot,flot=totals()
                    texto=f"📊 V38.4 6POS FINAL - 10PM\n📅 {now_mex.strftime('%d/%m/%Y')}\n💰 TOTAL NETO: ${tot:.2f}\nSaldo: ${data['b']:.2f}\nFlot NETO: {flot:+.2f}$\nHist NETO: ${data.get('gan_total',0):+.2f}\nCom: ${data.get('com_total',0):.2f}\nPos: {len(data['pos'])}/6 - 6POS $2000 libres\n"
                    for p in data["pos"]:
                        pr=P(p["sym"])
                        if pr==0:
                            pr=p["precio_entry"]
                        ...
                        if abs(pct)<25:
                            texto+=f"• {p['sym']} N{p.get('nivel',1)} Neto ${p.get('gan',0):+.2f} {pct:+.1f}%\n"
