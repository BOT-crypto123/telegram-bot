# V55.2 COMPLETO - MAQUINA DE HACER DINERO - FLOT + DESGLOSE REAL
...
def BOT1_compra(closes):
    ...
    return SI, top, lineas, f"BOT1 AYUDA {'✅ SI' if SI else '❌ NO'} {top['rebotes']}R {top['fuerza']:.0f}%" if SI else "BOT1 ❌ NO"

def BOT2_compra(closes, top, score):
    if not top: return False,0,0,"BOT2 ❌ NO - Sin linea"
    if not is_ny():
        rsi=rsi_calc(closes)
        return True, 3, 75, f"BOT2 🌙 ANALISTA ✅ RSI {rsi:.0f}"
    ...
    return SI, conf, f2, f"BOT2 NY 🟢 {'✅ SI' if SI else '❌ NO'} {conf}tq {f2:.0f}% RSI {rsi:.0f}"
...
def JEFE_compra(top, f2):
    ...
    return SI, score, bola, f"JEFE {'✅ SI' if SI else '❌ NO'} Score {score:.0f}% Bola ${bola}"
...
def BOT1_venta(tr, precio, closes):
    ...
    return V, f"B1 VENTA {'✅' if V else '❌'} {neto:.2f}%"
...
def decidir(s):
    ...
    for tr in ESTADO["open_trades"][:]:
        ...
        if (tend=="verde" and votos_v>=2) or (tend=="rojo" and votos_v>=2):
            ...
            ventas.append(f"💰 VENTA {tend} {votos_v}/3 {s} NETA {neto:+.2f}%")
    ...
    votos=sum([B1,B2,J3]); modo="NY 🟢" if is_ny() else "NOCHE 🌙"
    ...
    if B1 and B2 and J3:
        ...
        return {"tipo":f"COMPRA 3/3 💰 ${bola}","det":det+"\n✅ 3/3","closes":closes,"opens":opens,"highs":highs,"lows":lows,"lineas":lineas}
...
def dash():
    ...
    for tr in ESTADO["open_trades"]:
        ...
        cards+=f'<div style="background:#001a0a;border:1px solid #00ff88;padding:10px;margin:8px 0;border-radius:10px"><b>{tr["simbolo"]}</b> ${tr["bola"]}<br>Entrada {tr["entrada_real"]:.2f} -> {pa:.2f}<br>BRUTA {br:+.2f}% = ${gb:+.2f}<br>FEE -${fee:.2f}<br><b>NETA {ne:+.2f}% = ${gn:+.2f}</b></div>'
    pat=ESTADO["demo_balance"]+tb+fn
    h=f'<html><head><meta name="viewport" content="width=device-width"><meta http-equiv="refresh" content="20"><style>body{{background:#000;color:#fff;font-family:Arial;padding:10px}}.titulo{{background:#111;border:3px solid #FFD700;border-radius:16px;padding:18px;text-align:center}}.card{{background:#111;border:1px solid #222;border-radius:14px;padding:14px;margin:10px 0}}.flot{{background:#002200;border:2px solid #00ff88;border-radius:14px;padding:14px;margin:10px 0}}</style></head><body><div class="titulo"><h1>💰 V55.2 FLOT REAL 💰</h1></div><div class="flot">💳 EFECTIVO ${ESTADO["demo_balance"]:.2f} | EN BOLAS ${tb:.2f}<br>📈 FLOT BRUTO ${fb:+.2f}<br>💸 FEES A PAGAR -${ffees:.2f}<br>💰 FLOT NETO REAL ${fn:+.2f}<br>🏦 PAT TOTAL REAL ${pat:.2f}<br>📊 UTIL REAL ${pat-10000:+.2f} | FEES HIST ${ESTADO["fees"]:.2f}</div><div>{cards}</div>'
    for s in SYMBOLS:
...
def graf(s):
    ...
    ax.set_xlim(st,n); ax.set_title(f'{s} V55.2',color='white'); ax.tick_params(colors='white')
