...
def SUGERIR_RSI(sym, closes):
    ...
    rsi_now = RSI(closes)
    if vols > 2.5:
        return 45, f"Muy volatil {vols:.2f}% -> ponle 45. Ahora RSI {rsi_now:.0f}"
    elif vols > 1.5:
        return 40, f"Volatil {vols:.2f}% -> ponle 40. Ahora RSI {rsi_now:.0f}"
    else:
...
def ANALIZA(sym):
    ...
    if data.get("filtro_ema","ON") == "ON":
        ok = (r_now < limite) and p_ema_ok
    else:
        ok = (r_now < limite)
    mot = f"RSI {r_now:.1f} {'OK' if r_now<limite else 'NO'}<{limite:.0f}, P>EMA {'OK' if p_ema_ok else 'NO'}"
...
def prices():
    ...
    for sym in data["coins"]:
        ...
        out[sym]={"price":price,"rsi":round(rsi,1),"ema":round(ema,2),"action":action,"ok":ok,"motivo":mot,"activa":data["coins_activas"].get(sym,True),"limite":limite,"sug":sug, "p_ema_ok": p_ema_ok}
    return jsonify(out)
...
<div class=header><b style=font-size:20px;color:#ffcc00>💰 MÁQUINA BOLA DE NIEVE</b><div style=margin-top:8px;font-size:11px;background:#000;border-radius:8px;padding:6px;display:flex;justify-content:space-between"><span>USD/MXN $<span id=usdmxn>0</span></span><span id=cuantas>0/0</span><span>Win <span id=winrate>0%</span></span></div><div id=infoBola style=margin-top:8px></div></div>
<div class=config><div>TP: <select id=tp onchange="setTP()"><option value=0.3>0.1% NETO</option><option value=0.4>0.2% NETO</option><option value=0.5>0.3% NETO</option><option value=0.6>0.4% NETO</option></select></div><div>SL: <select id=sl onchange="setSL()"><option value=-1.0>-1%</option><option value=-1.5>-1.5%</option><option value=-2.0>-2%</option><option value=-3.0>-3%</option></select></div><div>RSI Venta: <select id=rsiV onchange="setRsiV()"><option value=68>68</option><option value=70>70</option><option value=72>72</option><option value=75>75</option></select></div><div>EMA: <select id=emaF onchange="setEmaF()"><option value="ON">ON</option><option value="OFF">OFF</option></select></div><div>RSI Compra: <select id=rsiGlobal onchange="setRsiGlobal()"><option value=32>32</option><option value=35>35</option><option value=38>38</option><option value=40>40</option><option value=45>45</option><option value=50>50</option></select></div><div>Bolas: <select id=maxEnt onchange="setMax()"><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div><div><button class=btn btn-g id=autoBtn onclick="toggleAuto()" style=width:auto>...</button></div></div>
<div class=sug-box id=sugBox style="border:2px solid #00ff88"><b style=color:#00ff88>ROBOT - QUE ES LO MEJOR HOY</b><div id=sugList style="margin-top:6px">Cargando...</div></div>
<div id=grid class=grid></div>
<table><thead><tr><th>Moneda</th><th>Entry</th><th>Ahora</th><th>Neta</th><th>Accion</th></tr></thead><tbody id=tbody></tbody></table>
<script>
async function load(){
 ...
 let cerebro = caidas>=6? `CAIDA FUERTE ${caidas}/${Object.keys(d).length} -> LO MEJOR: SL -2% o -3% | RSI Venta 70 | EMA OFF` : caidas>=3? `LATERAL ${caidas} -> SL -2% | Venta 72 | EMA ON` : `ALCISTA -> SL -1.5% | Venta 75 | EMA ON`;
    ...
    while True:
        try:
            for p in list(data["pos"]):
                ...
                if data["auto"] and (gan_bruta>=data["tp_bruto"] or gan_bruta<=data.get("sl_pct",-2.0) or RSI(C(p['sym']))>=data.get("rsi_venta",72.0)):
            ...
            if data["auto"] and len(data["pos"]) < data["max_entradas"]:
                for sym in data["coins"]:
                    if not data["coins_activas"].get(sym,True):
                        continue
                    if any(p['sym']==sym for p in data["pos"]):
                        continue
                    ok, rsi, ema, mot, lim, sug, p_ok = ANALIZA(sym)
