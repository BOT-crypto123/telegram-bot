...
prices={}
rsis={"BTC":38,"ETH":42,"SOL":43,"XAUUSD":40,"NVDA":50,"TSLA":50}
...
def load():
    ...
    try:
        ...
        if b<3000 and len(pos)>=4:
            data={"b":5000,"pos":[],"alert_users":r.get("alert_users",[]),"auto":True,"gan_total":0,"com_total":0}
            save()
            return
        data["b"]=r.get("b",5000)
        data["pos"]=r.get("pos",[])
        ...
        try:
            for s in ["BTC","ETH","SOL","XAUUSD","NVDA","TSLA"]:
                ...
                for p in list(data["pos"]):
                    ...
                    if pct>=TP or pct<=-SL:
                        neto=p["amt"]*pct/100-p["amt"]*0.006
                        data["b"]=data["b"]+p["amt"]+neto
                        data["pos"].remove(p)
                        save()
                if not data["auto"]:
                    continue
