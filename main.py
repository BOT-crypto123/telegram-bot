...
def totals():
    ...
    for p in data["pos"]:
        entry=get_entry(p)
        pr=prices.get(p["sym"], entry)
        if pr>0 and entry>0:
            p["gan"]=((pr-entry)/entry)*get_monto_p(p)
...
def dash():
    try:
        tot,flot=totals(); col="#00ff88" if flot>=0 else "#ff4444"
        pos_html=""
        for p in data["pos"]:
            entry=get_entry(p); monto=get_monto_p(p)
            pr=prices.get(p["sym"], entry)
            pct=((pr-entry)/entry*100) if entry else 0
            pos_html+=f"<div onclick=\"location='/chart/{p['sym']}'\" style='display:flex;justify-content:space-between;padding:14px;border-bottom:1px solid #222'><div><b>{p['sym']} N{p.get('nivel',1)} ${monto}</b> {pct:+.1f}%<br><small>TP ${entry*1.013:.1f} SL ${entry*0.82:.1f}</small></div><span style='color:{col}'>{p.get('gan',0):+.2f}$</span></div>"
        ...
        try:
            if data.get("auto"):
                for p in list(data["pos"]):
                    ...
                    if pr==0 or entry==0: continue
                    pct=(pr-entry)/entry*100
                    max_p=p.get("max_price",pr)
                    if pct>=1.3 or (pct>4 and pr < max_p*0.97) or pct<=-18:
                        gan=((pr-entry)/entry)*monto
                        data["b"]+=monto+gan; data["gan_total"]+=gan; data["pos"].remove(p); save()
                for sym in ALL_COINS:
                    ...
                    if count==0 and rsi<32: should=True
                        ...
                        entries=[get_entry(x) for x in data["pos"] if x["sym"]==sym]
                        avg=sum(entries)/len(entries) if entries else pr
