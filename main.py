...
def get_binance_price(s):
    try: return float(requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={BINANCE[s]}", timeout=5).json()['price'])
    except: return None

def get_binance_klines(s):
    try:
        r=requests.get(f"https://api.binance.com/api/v3/klines?symbol={BINANCE[s]}&interval=15m&limit=100", timeout=6).json()
        df=pd.DataFrame(r, columns=['t','o','h','l','c','v','a','b','c','d','e','f']); df['Close']=df['c'].astype(float)
        df['t']=pd.to_datetime(df['t'], unit='ms'); df=df.set_index('t'); return df
    except: return None
...
def get_data():
    ...
    for k in SYMBOLS:
        pr=None
        if k in BINANCE:
            pr=get_binance_price(k)
            if pr:
                df=get_binance_klines(k); r=float(rsi(df['Close']).iloc[-1]) if df is not None else 50
                prices[k]=pr; rsis[k]=r
        if pr is None:
            try:
                df=yf.download(SYMBOLS[k], period="1d", interval="5m", progress=False, auto_adjust=True, threads=False)
                if len(df)>5: prices[k]=float(df['Close'].iloc[-1]); rsis[k]=float(rsi(df['Close']).iloc[-1])
            except: pass
        if k not in prices: prices[k]=0; rsis[k]=0
...
def chart_bytes(sym):
    ...
    try:
        ...
        ax1.plot(df['Close'],linewidth=2); ax1.plot(df['SMA20'],linewidth=1,alpha=.6); ax1.plot(df['SMA50'],linewidth=1,alpha=.6)
        ax1.scatter(df[df['RSI']<45].index, df[df['RSI']<45]['Close'], marker='^', color='green', s=60)
        ax1.set_title(f"{sym} ${price:.2f} RSI {rsi_v:.1f} AUTO {'ON' if AUTO_ON else 'OFF'}"); ax1.grid(True,alpha=.3)
...
HTML="""<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="30">
...
<div class="top"><div><b>MAQUINA V45.3 FIX 409</b><br><small>Cap $5000 Bola $500/750/1000</small></div><a href="/toggle"><button class="{{'on' if auto else 'off'}}">AUTO {{'ON' if auto else 'OFF'}}</button></a></div>
<p>{{hora}} | BTC/ETH/SOL 24/7 | WEBHOOK MODE</p><div class="grid">
...
def home():
    ...
    return render_template_string(HTML, syms=SYMBOLS.keys(), auto=AUTO_ON, hora=datetime.now(TZ_MX).strftime("%H:%M:%S"),
        prices={k:f"{v:.2f}" if v>0 else "--" for k,v in pr.items()}, rsis={k:f"{v:.1f}" if v>0 else "--" for k,v in rs.items()})

@app.route('/toggle')
def tog(): global AUTO_ON; AUTO_ON=not AUTO_ON; return redirect('/')
@app.route('/chart/<sym>')
def ch(sym): d=chart_bytes(sym.upper()); return Response(d,mimetype='image/png') if d else ("Generando grafica, recarga en 10s",503)
...
def start(m):
    ...
    bot.send_message(m.chat.id,f"✅ V45.3 WEBHOOK - FIX 409\n{'🟢AUTO ON' if AUTO_ON else '🔴AUTO OFF'}\n{URL}",reply_markup=kb)
...
# FIX 409 - set webhook una sola vez
@app.before_first_request
def sethook():
    if URL:
        time.sleep(2); bot.remove_webhook()
        time.sleep(1); bot.set_webhook(url=f"{URL}/{TOKEN}")
        print(f"Webhook seteado: {URL}/{TOKEN}")

if __name__!= "__main__":
    # para gunicorn/render
    if URL:
        try: bot.remove_webhook(); time.sleep(1); bot.set_webhook(url=f"{URL}/{TOKEN}"); print("Webhook OK")
