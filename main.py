...
def chalan_confirma(sym, precio, df):
    ...
    try:
        ...
        if df is None or len(df)<30:
            ...
            fig, ax=plt.subplots(figsize=(12,6)); ax.text(0.5,0.5,f"{sym} ${price:.2f}\nMAQUINA DE HACER DINERO {days}D", ha='center', va='center', fontsize=18, transform=ax.transAxes); ax.set_axis_off()
    ...
    except Exception as e: print(f"chart {sym} {e}"); return None
def trading_loop():
    while True:
        try:
...
try:
    if URL: bot.remove_webhook(); time.sleep(1); bot.set_webhook(url=f"{URL}/{TOKEN}"); print("WEBHOOK V47 OK - MAQUINA DE HACER DINERO + CHALAN")
