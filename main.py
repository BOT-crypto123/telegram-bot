...
def dash():
    ...
    if not pos_html: pos_html="Sin posiciones - Esperando RSI<32 + EMA*0.995 + BTC>-1.5%"
...
<div class=pos><b>Posiciones ({len(data['pos'])}/5) - Auto: <span id=autoPos>...</span> | Lógica: RSI&lt;32 + Precio&gt;EMA*0.995 + BTC&gt;-1.5%</b><br><br><div id=pos>{pos_html}</div><br><small>V28.2 MILLONARIO - XRP 29.4 ya entra | /dashboard en Telegram</small></div>
<script>async function L(){try{let st=await fetch('/api/status').then(r=>r.json());let btn=document.getElementById('autoBtn');let txt=document.getElementById('autoTxt');let posTxt=document.getElementById('autoPos');btn.innerText=st.auto_buy?'🤖 AUTO ON':'⏸️ AUTO OFF';btn.style.background=st.auto_buy?'#00ff88':'#ff4444';btn.style.color='#000';txt.innerText=st.auto_buy?'Millonario activo':'Manual con filtro';posTxt.innerText=st.auto_buy?'ON':'OFF';posTxt.style.color=st.auto_buy?'#00ff88':'#ff4444';let r=await fetch('/api/prices');let d=await r.json();let h='';for(let s in d){let cls=d[s].action=='COMPRAR'?'buy':d[s].action=='VENDER'?'sell':'wait';let filtTxt=d[s].filt?(d[s].rsi<32?'✅ MILLONARIO':'✅ Filtros OK'):'⏸️ BLOQUEADO';let btnHtml=d[s].action=='COMPRAR'?`<a href=/buy/${{s}}><button class=btn g>COMPRAR $50</button></a>`:`<a href=/chart/${{s}}><button class=btn style=background:#333;color:#fff>VER GRAFICA</button></a>`;h+=`<div class=card ${{cls}} onclick="location='/chart/${{s}}'"><b>${{s}} $${{d[s].price.toFixed(2)}}</b><span class=score>${{d[s].score}}/5</span><br><small>RSI ${{d[s].rsi}} | EMA $${{d[s].ema20}}<br>BTC ${{d[s].btc_t}}% | ${{filtTxt}}</small><br><span style=background:#ffcc00;color:#000;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:bold>${{d[s].action}}</span>${{btnHtml}}</div>`;}document.getElementById('g').innerHTML=h;}catch(e){}}L();setInterval(L,25000);</script></body></html>"""
@app.route('/buy/<sym>')
def buy(sym):
...
def wh():
    ...
    if "message" in d:
        ...
        if "DASHBOARD" in txt:
            tg(chat,f"📊 TU DASHBOARD MILLONARIO:\n{dash_url}\nToca para abrir\n\nSi no abre, Render está dormido, espera 30 seg y toca de nuevo", markup)
            return {"ok":True}
        if "FORCE" in txt:
            ...
            if sym in data["coins"] and len(data["pos"])<5 and not any(p['sym']==sym for p in data["pos"]):
                ...
                data["pos"].append({"sym":sym,"monto":50,"gan":0,"precio_entry":price,"max_price":price}); data["b"]-=50; data["trades_hoy"]+=1; save()
                tg(chat,f"⚠️ {sym} FORZADO ${price:.2f} RSI {rsi:.1f} - SIN FILTRO", markup)
            return {"ok":True}
        if txt in data["coins"]:
            if len(data["pos"])>=5: tg(chat,f"⚠️ Ya tienes 5 posiciones", markup)
            elif any(p['sym']==txt for p in data["pos"]): tg(chat,f"⚠️ Ya tienes {txt}", markup)
            else:
                ...
                if rsi<32 and filt:
                    ...
                    tg(chat,f"⏸️ {txt} BLOQUEADO RSI {rsi:.1f} EMA ${ema20:.1f} BTC {btc_t:.2f}%\nFilt EMA:{'OK' if price>ema20*0.995 else 'NO'} BTC:{'OK' if btc_t>-1.5 else 'NO'}\nUsa {txt} FORCE para forzar", markup)
        if "/AUTO" in txt:
            if "ON" in txt: data['auto_buy']=True
            elif "OFF" in txt: data['auto_buy']=False
            else: data['auto_buy']=not data.get('auto_buy',True)
            save(); tg(chat,f"{'✅ AUTO ON' if data['auto_buy'] else '⏸️ AUTO OFF'}", markup)
        if "/STATUS" in txt or "/ESTADO" in txt:
            total,val=totals(); tg(chat,f"🤖 V28.2\nAuto: {'ON ✅' if data.get('auto_buy',True) else 'OFF ⏸️'}\nTotal ${total:.2f}\nPos {len(data['pos'])}/5\nDash: {dash_url}", markup)
        if "/REPORTE" in txt or "PORTAFOLIO" in txt:
            total,val=totals(); pos_txt="\n".join([f"{p['sym']} {p.get('gan',0):.2f}%" for p in data['pos']]); tg(chat,f"📊 V28.2\nSaldo ${data['b']:.2f} Total ${total:.2f}\nGan ${data['gan_total']:.2f}\nAuto {'ON' if data.get('auto_buy',True) else 'OFF'}\nPos:\n{pos_txt if pos_txt else 'Vacío'}\nDash: {dash_url}", markup)
        if "/START" in txt:
            tg(chat,f"V28.2 MILLONARIO FINAL\n📊 Dashboard: {dash_url}\nLógica: RSI<32 + EMA*0.995 + BTC>-1.5%\nBotones BTC ETH SOL XRP\nEscribe DASHBOARD para link\nBTC = con filtro\nBTC FORCE = sin filtro\n/auto on/off /status", markup)
        save()
    return {"ok":True}
def auto_loop():
    time.sleep(5)
