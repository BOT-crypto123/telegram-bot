...
def CEREBRO(sym, closes):
    ...
    rsi=RSI(closes); vols=sum(abs(closes[i]-closes[i-1])/closes[i-1] for i in range(-30,0))/30*100
...
def ANALIZA(sym):
    closes=C(sym)
    if len(closes)<30:
        pr=P(sym); return False,50.0,pr,f"Sin datos ({len(closes)})",35,"","Sin datos"
...
def get_usdmxn():
    try: r=requests.get("https://api.exchangerate-api.com/v4/latest/USD",timeout=4).json(); return float(r["rates"]["MXN"])
    except:
...
def prices():
    ...
    for sym in data["coins"]:
        ok,rsi,ema,mot,limite,sug,cerebro=ANALIZA(sym); closes=C(sym); price=closes[-1] if closes else P(sym)
...
def state():
    ...
    for p in data["pos"]:
        price=P(p["sym"]); p["ahora"]=price; gan_bruta_pct=((price-p["entry"])/p["entry"]*100) if p["entry"] else 0
        gan_bruta_mxn=p["monto"]*gan_bruta_pct/100; com=p["monto"]*FEE_ENTRADA+(p["monto"]+gan_bruta_mxn)*FEE_SALIDA
        p["gan_bruta_pct"]=gan_bruta_pct; p["gan_neta_pct"]=gan_bruta_pct-FEE_TOTAL*100; p["gan_bruta_mxn"]=gan_bruta_mxn; p["gan_neta_mxn"]=gan_bruta_mxn-com
...
def sell_api(sym):
    ...
    for p in data["pos"][:]:
        if p["sym"]==sym:
            price=P(sym); gan_bruta_pct=(price-p["entry"])/
