...
def rsi_calc(s,p=14):
    try:
        d=s.diff(); g=d.where(d>0,0).rolling(p).mean(); l=-d.where(d<0,0).rolling(p).mean(); rs=g/l
        return 100-(100/(1+rs))
...
def get_all_data(days=3):
    ...
    if days==3: data_cache.update({"prices":prices,"rsis":rsis,"dfs":dfs,"time":time.time()})
...
def analizar_entrada_con_lineas(sym, precio, df):
    ...
    for lin in lineas:
        ...
        if dist < 0.6:
            ...
            if fuerza > 60:
                mejor = {'linea': lin['precio'], 'rebotes': lin['rebotes'], 'fuerza': fuerza, 'dist': dist, 'total_lineas': len(lineas)}
                break
    return mejor
...
def chart_bytes_pro(sym, days=3):
    ...
    if cache_key in chart_cache and now-chart_cache[cache_key][0]<25:
    ...
    try:
        ...
        if df is None or len(df)<30:
            ...
            ax.text(0.5,0.5,f"{sym} ${price:.2f}\nMAQUINA DE HACER DINERO {days}D", ha='center', va='center', fontsize=18, transform=ax.transAxes)
            ax.set_axis_off()
        ...
        entradas=df[df['RSI']<44]; ax1.scatter(entradas.index, entradas['Close'], marker='^', color='#00ff00', s=100, edgecolors='black', zorder=5)
        ...
        if pos: ax1.axhline(pos['entry'], color='#f1c40f', ls='-', lw=2.5, label=f"ENTRADA ${pos['entry']:.2f}")
            ...
            for sym in ALL_SYMS:
                ...
                if sym in positions:
                    ...
                    if profit>=TP_PCT or trail_drop>=TRAIL_PCT or profit<=SL_PCT:
                        ...
                        del positions[sym]; save_json(POS_FILE, positions); notify(f"{'GANANCIA' if is_win else 'PERDIDA'} {sym} Gan ${ganancia:.2f} Total ${gains['total']:.2f}"); continue
                    ...
                    if analisis and analisis['fuerza']>68 and r<50:
                        ...
                        if sym not in positions:
                            if conf['confirma'] and conf['fuerza']>70:
                                bola_final = min(b2*2.2, TOPE_B2*1.5); strat = f"V47 DOBLE CANDADO {analisis['rebotes']}R+{conf['rebotes']}R F{analisis['fuerza']:.0f}%+{conf['fuerza']:.0f}%"
                                notify(f"DOBLE CANDADO {sym} Linea ${analisis['linea']:.2f} MAQ {analisis['rebotes']}R {analisis['fuerza']:.0f}% + CHALAN {conf['rebotes']}R {conf['fuerza']:.0f}% -> Bola GRANDE ${bola_final}")
                            else:
                                bola_final = b1; strat = f"V47 SOLO MAQUINA {analisis['rebotes']}R F{analisis['fuerza']:.0f}%"
                                notify(f"MAQUINA SOLA {sym} Linea ${analisis['linea']:.2f} {analisis['rebotes']}R {analisis['fuerza']:.0f}% Chalan NO -> Bola chica ${bola_final}")
                            positions[sym]={'entry':pr, 'max':pr, 'bola':bola_final, 'strat':strat, 'time':str(datetime.now(TZ_MX))}; save_json(POS_FILE, positions)
                    elif r<RSI_BUY and len([s for s in positions if s in ["BTC","ETH","SOL","XAUUSD"]])<2:
                        count=len([s for s in positions if s in ["BTC","ETH","SOL","XAUUSD"]]); bola=b1 if count==0 else b2
                        if sym not in positions:
                            positions[sym]={'entry':pr, 'max':pr, 'bola':bola, 'strat':f'BolaNieve RSI', 'time':str(datetime.now(TZ_MX))}; save_json(POS_FILE, positions); notify(f"ENTRADA RSI {sym} ${pr:.2f} RSI {r:.1f} Bola ${bola}")
                    ...
                    if sym in ny_data and ny_data[sym].get('date')==today and 'high' in ny_data[sym]:
                        ...
                        if 9<=now_ny.hour<=11 and not ny_data[sym].get('e1_done', False) and sym not in positions:
                            if pr>high*(1+NY_PCT/100) or pr<low*(1-NY_PCT/100):
                                positions[sym]={'entry':pr, 'max':pr, 'bola':bny, 'strat':'E1 Break CHALAN', 'time':str(datetime.now(TZ_MX))}; save_json(POS_FILE, positions); ny_data[sym]['e1_done']=True; save_json(NY_FILE, ny_data); notify(f"ENTRADA NY E1 CHALAN {sym} High ${high:.2f} Low ${low:.2f} Bola NY ${bny}")
                        if 11<=now_ny.hour<15 and sym not in positions:
                            if pr>t_high or pr<t_low:
                                positions[sym]={'entry':pr, 'max':pr, 'bola':bny, 'strat':'E2 Triple CHALAN', 'time':str(datetime.now(TZ_MX))}; save_json(POS_FILE, positions); notify(f"ENTRADA NY E2 TRIPLE CHALAN {sym} Triple ${t_high:.2f}/${t_low:.2f} Bola ${bny}")
...
<div class="top"><div><b style="color:#00ff88;font-size:16px">MAQUINA DE HACER DINERO + CHALAN V47</b><br><small>Bola ${{b1}}/{{b2}} NY ${{bny}} | Gan ${{"%.2f"|format(gains.total)}} | Perd {{gains.loss_streak}}</small></div><a href="/toggle"><button class="{{'on' if auto else 'off'}}">AUTO {{'ON' if auto else 'OFF'}}</button></a></div>
<p>{{hora}} | V47 DOBLE CANDADO AUTO-LINEAS</p><div class="grid">
...
<div class="box">
<b style="color:#00ff88">V47 DOBLE CANDADO</b><br>
MAQUINA analiza + CHALAN confirma = Bola GRANDE<br>
{% if positions %}{% for s,p in positions.items() %} {{s}} [{{p.strat}}]: Ent ${{"%.2f"|format(p.entry)}} | Ahora ${{prices[s]}} | Bola ${{p.bola}}<br>{% endfor %}{% else %}Sin posiciones - MAQUINA + CHALAN escaneando lineas...<br>{% endif %}<br>
<b>GANANCIA:</b> <span class="gain">${{"%.2f"|format(gains.total)}} en {{gains.trades}} trades</span>
</div></body></html>"""
...
<a class="back" href="/">Volver V47</a><h2>{{sym}} - V47 DOBLE CANDADO</h2>
<div class="btns"><a id="btn1" class="btn" onclick="setDays(1)">1 DIA</a><a id="btn3" class="btn active" onclick="setDays(3)">3 DIAS</a><a id="btn5" class="btn" onclick="setDays(5)">5 DIAS</a><a id="btn8" class="btn" onclick="setDays(8)">8 DIAS</a></div>
<img id="chart" src="/chart/{{sym}}?d=3" />
<p><small>V47 Lineas verdes/naranjas | Bola ${{b1}}/{{b2}} NY ${{bny}} | Gan ${{gain}}</small></p>
</body></html>"""
...
def allh(m):
    ...
    if t in ALL_SYMS: b1,b2,bny,total=get_bolas_snowball(); bot.send_message(m.chat.id, f"{URL}/view/{t}\nV47 DOBLE CANDADO"); d=chart_bytes_pro(t, days=3); pr,rs,_=get_all_data(); bot.send_photo(m.chat.id,d,caption=f"V47 {t} ${pr.get(t,0):.2f} RSI {rs.get(t,0):.1f} | Lineas auto") if d else None

try:
    if URL: bot.remove_webhook(); time.sleep(1); bot.set_webhook(url=f"{URL}/{TOKEN}"); print("WEBHOOK V47 OK")
