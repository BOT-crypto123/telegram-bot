# V55.3 - MAQUINA DE HACER DINERO - CON RESET + FLOT REAL
...
def BOT2_compra(closes, top, score):
    if not top: return False,0,0,"BOT2 ❌"
    if not is_ny(): return True, 3, 75, f"BOT2 🌙 ✅ RSI {rsi_calc(closes):.0f}"
    ...
    return SI, conf, f2, f"BOT2 NY {'✅' if SI else '❌'} {f2:.0f}% RSI {rsi:.0f}"
def JEFE_compra(top, f2):
    if not top: return False,0,0,"JEFE ❌"
    ...
    return SI, score, bola, f"JEFE {'✅' if SI else '❌'} {score:.0f}% ${bola}"
def BOT1_venta(tr, precio, closes):
    ...
    V=neto>=TRAILING_PCT or neto<=-4.32 or (max_neto>=TRAILING_PCT and neto<=max_neto-TRAILING_PCT); return V, f"B1 {'✅' if V else '❌'} {neto:.2f}%"
def BOT2_venta(tr, precio, closes):
    ...
    V=neto>=TRAILING_PCT or neto<=-4.32 or (max_neto>=TRAILING_PCT and rsi<60) or rsi>72; return V, f"B2 {'✅' if V else '❌'} {neto:.2f}%"
def JEFE_venta(tr, precio):
    bruto=(precio-tr["entrada_real"])/tr["entrada_real"]*100; neto=bruto-FEE_RT*100; max_neto=tr.get("max_neto",neto)
    V=neto>=TRAILING_PCT or neto<=-4.32 or (max_neto>=TRAILING_PCT and neto<=0.20); return V, f"JEFE {'✅' if V else '❌'} {neto:.2f}%"
def decidir(s):
    ...
    for tr in ESTADO["open_trades"][:]:
        ...
        if sum([v1][v2][v3])>=2 and ((tend=="verde" and sum([v1][v2][v3])>=2) or (tend=="rojo" and sum([v1][v2][v3])>=2)):
    ...
    if count_symbol>=2: return {"tipo":"MAX 2 HOY ESTE SIMBOLO 🔒","det":det,"closes":closes,"opens":opens,"highs":highs,"lows":lows,"lineas":lineas}
    if not is_ny() and s not in ["BTC","ETH","SOL"]: return {"tipo":"NOCHE PAUSA ⏸️","det":det,"closes":closes,"opens":opens,"highs":highs,"lows":lows,"lineas":lineas}
    if B1 and B2 and J3:
...
def dash():
    ...
    h=f'<html><head><meta name="viewport" content="width=device-width"><meta http-equiv="refresh" content="20"><style>body{{background:#000;color:#fff;font-family:Arial;padding:10px}}.titulo{{background:#111;border:3px solid #FFD700;border-radius:16px;padding:18px;text-align:center}}.flot{{background:#002200;border:2px solid #00ff88;border-radius:14px;padding:14px;margin:10px 0}}.card{{background:#111;border:1px solid #222;border-radius:14px;padding:14px;margin:10px 0}}</style></head><body><div class="titulo"><h1>💰 V55.3 MAQUINA DINERO 💰</h1></div><div class="flot">💳 ${ESTADO["demo_balance"]:.2f} | BOLAS ${tb:.2f}<br>BRUTO ${fb:+.2f} | FEES -${ffees:.2f} | NETO ${fn:+.2f}<br>PAT ${pat:.2f} | UTIL ${pat-10000:+.2f} | FEES HIST ${ESTADO["fees"]:.2f}</div><div>{cards}</div>'
    for s in SYMBOLS:
...
def graf(s):
    ...
    ax.set_xlim(st,n); ax.set_title(f'{s} V55.3',color='white'); ax.tick_params(colors='white')
...
def all_msg(m):
    ...
    if "RESET" in t:
        ...
        save_estado()
        bot.send_message(m.chat.id, "✅ RESET COMPLETO\n💳 $10,000\n📦 $0 EN BOLAS\n💸 FEES $0\nMAQUINA LISTA PARA HACER DINERO 💰")
        return
    if "DASHBOARD" in t or t=="DASH":
        url=os.getenv("RENDER_EXTERNAL_URL") or "";
        if url and not url.startswith("http"): url="https://"+url
        bot.send_message(m.chat.id, f"🌐 DASHBOARD V55.3:\n{url or 'Tu link.onrender.com'}"); return
    if "BALANCE" in t or "FLOT" in t or "DESGLOSE" in t:
        tb=0; fb=0; fn=0; ffees=0; detalle=""
        for tr in ESTADO["open_trades"]:
            pa=get_price(tr["simbolo"]); br=(pa-tr["entrada_real"])/tr["entrada_real"]*100; gb=tr["bola"]*br/100; fee=tr["bola"]*FEE_RT; ne=br-FEE_RT*100; gn=tr["bola"]*ne/100
            tb+=tr["bola"]; fb+=gb; fn+=gn; ffees+=fee
            detalle+=f"\n{tr['simbolo']} ${tr['bola']}: Bruta {br:+.2f}% ${gb:+.2f} Fee -${fee:.2f} NETA {ne:+.2f}% ${gn:+.2f}\n"
        pat=ESTADO["demo_balance"]+tb+fn
        msg=f'''💰 V55.3 DESGLOSE REAL
