...
app = Flask(__name__)
@app.route('/')
def home(): return f"V39.1 FIX LIVE"
...
async def alert_loop(application):
    ...
    while True:
        try:
            ...
            print(f"Check: BTC {btc_rsi:.1f} ETH {eth_rsi:.1f} XRP {xrp_rsi:.1f}")
            for uid, u in data["users"].items():
                ...
                for moneda, rsi_val in [("BTC",btc_rsi), ("ETH",eth_rsi), ("XRP",xrp_rsi)]:
                    ...
                    if time.time() - ultima < 3600*4: continue
                    if rsi_val < 30:
                        try:
                            await application.bot.send_message(chat_id=int(uid), text=f"ALERTA COMPRA {moneda} RSI {rsi_val:.1f} SOBREVENDIDO")
                            u["ultima_alerta"][moneda]=time.time()
                        except: pass
                    elif rsi_val > 70:
                        try:
                            await application.bot.send_message(chat_id=int(uid), text=f"ALERTA VENTA {moneda} RSI {rsi_val:.1f} SOBRECOMPRADO")
                            u["ultima_alerta"][moneda]=time.time()
                        except: pass
            save_data(data)
