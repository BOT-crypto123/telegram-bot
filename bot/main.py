...
COMISION = 0.0078
ARCHIVO_TRADES = "trades.json" # Aquí se guarda tu simulación

PRECIO_BTC = float(os.getenv("PRECIO_BTC") or 64364)
...
def job():
    ...
    for coin_id, info in MONEDAS.items():
        ...
        # Botones de SIMULACIÓN - ya no te sacan a Bitso/Binance
        ...
        markup.add(
            ...
            types.InlineKeyboardButton(f"📊 Ver mi balance", callback_data=f"balance_{simbolo}")
        )
        ...
        # 1. ALERTA COMPRA
        if change <= -2:
            texto = f"🟢 ALERTA COMPRA {simbolo}!\n📉 Bajó {change:.2f}%\n💰 Precio: ${price:,.2f}\n💵 Tu precio compra sim: ${precio_compra_real:,.2f}\n\nPÉRDIDA/GANANCIA ACTUAL: ${ganancia_real:+,.2f} ({porc_real:+.2f}%)"
            bot.send_message(CHAT_ID, texto, reply_markup=markup)
            continue

        # 2. ALERTA VENTA
        if porc_real >= 2:
            texto = f"🔴 ALERTA VENTA {simbolo}!\n📈 Subió {change:+.2f}%\n💰 Precio: ${price:,.2f}\n✅ GANANCIA NETA SIMULADA: ${ganancia_real:.2f} ({porc_real:.2f}%)\nBruta: ${price - precio_compra_real:.2f} - Comisiones: ${com_c+com_v:.2f}"
            bot.send_message(CHAT_ID, texto, reply_markup=markup)

# --- MANEJO DE LOS BOTONES SIMULADOS ---
...
def callback(call):
    ...
    if call.data.startswith("buy_"):
        ...
        bot.answer_callback_query(call.id, f"Simulaste compra de {simbolo} a ${precio_actual}")
        bot.send_message(CHAT_ID, f"✅ SIMULACIÓN: Compraste {simbolo} a ${precio_actual:,.2f}\nAhora verás tu ganancia/pérdida real con este precio.")
        ...
        bot.send_message(CHAT_ID, f"💰 SIMULACIÓN VENTA {simbolo}\nCompraste en: ${precio_compra:,.2f}\nVendiste en: ${precio_actual:,.2f}\n\n{'🟢 GANANCIA TOTAL NETA' if ganancia>0 else '🔴 PÉRDIDA TOTAL NETA'}: ${ganancia:+,.2f} ({porc:+.2f}%)")
...
if __name__ == "__main__":
    # Si lo ejecuta Railway como cron, solo hace job()
    # Si lo ejecutas en local con polling, responde a botones
    job()
    # Descomenta esto si quieres que los botones funcionen 24/7
