...
app = Flask(__name__)
@app.route('/')
def home(): return "V36.7 SYNTAX FIX OK"
...
def start_bot_thread():
    ...
    async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if d=="menu_tp":
            ...
            return
        if d.startswith("sl_"):
            u["stoploss"]=float(d.split("_")[1])
            data["users"][uid]=u
            save_data(data)
            await q.edit_message_text(texto(u), reply_markup=kb_main(u))
            return
        if d.startswith("tp_"):
            u["takeprofit"]=float(d.split("_")[1])
            data["users"][uid]=u
            save_data(data)
            await q.edit_message_text(texto(u), reply_markup=kb_main(u))
            return
        if d.startswith("c_"):
            ...
            if u['efectivo']<100:
                ...
                return
            qty=(100/fx)/precios[mon]
            u[mon]+=qty
            u['efectivo']-=100
            u['precio_compra'][mon]=precios[mon]
            data["users"][uid]=u
            save_data(data)
            await q.edit_message_text(texto(u), reply_markup=kb_main(u))
            return
        if d.startswith("v_"):
            mon=d.split("_")[1]
            mxn=u[mon]*precios[mon]*fx
            u[mon]=0
            u['efectivo']+=mxn
            data["users"][uid]=u
            save_data(data)
            await q.edit_message_text(f"Vendido {mon.upper()} ${mxn:.2f}\n\n{texto(u)}", reply_markup=kb_main(u))
            return
        if d=="grafica":
            await q.edit_message_text("Generando grafica 7 dias exactos... 5s")
            path=crear_grafica()
            with open(path,'rb') as f:
                await q.message.reply_photo(photo=f, caption="Ultimos 7 dias BTC/ETH/XRP % - Coinbase")
