...
def send_chart(chat_id, symbol):
    try:
        ...
        # Config grafica negra
        cfg = {
            "type":"line",
            "data":{"labels":[""]*len(prices),"datasets":[{"data":prices,"borderColor":"#00ff88","backgroundColor":"rgba(0,255,136,0.2)","fill":True,"pointRadius":0,"borderWidth":2}]},
            "options":{"legend":{"display":False},"scales":{"xAxes":[{"display":False}],"yAxes":[{"ticks":{"fontColor":"white"}}]},"title":{"display":True,"text":f"{symbol} 24h {round(prices[-1],4)} USD","fontColor":"white","fontSize":16}}
        }
        enc = urllib.parse.quote(json.dumps(cfg))
        chart_url = f"https://quickchart.io/chart?c={enc}&bkg=black&width=600&height=300"

        urlp = f"https://api.telegram.org/bot{TOKEN}/sendPhoto"
        requests.post(urlp, data={"chat_id": chat_id, "caption": f"GRAF NEGRA {symbol} {round(prices[-1],4)} - {VERSION}", "photo": chart_url}, timeout=15)
    except Exception as e:
        print("chart err", e)
        send_msg(chat_id, f"Error graf {symbol} intenta otra vez")
        ...
        if "message" in data:
            ...
            if txt in SYMBOLS:
                ...
                if e:
                    ...
                    send_msg(chat_id, f"{VERSION} {txt} {round(p,4)} Sin partida - COMPRAR?")
                return "ok",200
            ps = get_price(SELECTED)
            if "GRAF" in txt:
                send_msg(chat_id, f"Generando grafica negra {SELECTED}...")
                ...
                ENTRIES[SELECTED] = ps
                send_msg(chat_id, f"PARTIDA {SELECTED} {round(ps,4)} SL:{round(ps*0.95,4)} TP:{round(ps*1.10,4)}")
            elif "VENDER" in txt:
