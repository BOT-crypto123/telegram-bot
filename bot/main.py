...
def callback(call):
    ...
    if call.data == "comprar":
        if cartera["usd"] < 10:
            bot.answer_callback_query(call.id, "Sin USD")
            return
        usd = cartera["usd"] * 0.5
        com = usd * COMISION
        ...
        if cartera["btc"] <= 0:
            bot.answer_callback_query(call.id, "Sin BTC")
            return
    ...
    while True:
        ...
        try:
            ...
            if porc_real >= 2.0 and (ahora - ultima_alerta_venta) > 7200:
                bot.send_message(int(CHAT_ID), f"🔴 *OPORTUNIDAD VENTA GANANDO!*\n💰 ${precio:,.2f}\n✅ NETA: ${ganancia_real:.2f} ({porc_real:.2f}%)\nComisión c: ${com_c:.2f} / v: ${com_v:.2f}", reply_markup=get_keyboard(), parse_mode="Markdown")
                ultima_alerta_venta = ahora
        except Exception as e:
            print(f"Error oportunidad: {e}")

# --- REPORTE DIARIO 10:00 PM HORA NOGALES ---
def reporte_diario():
        ...
        try:
            ...
            if ahora_mx.hour == 22 and ahora_mx.minute == 0:
                ...
                if fecha_hoy != ultimo_reporte_fecha:
                    ...
                    # Ganancia real con tu precio de 64364
                    ...
                    reporte = (
                        f"📅 *REPORTE DIARIO 10:00 PM - {ahora_mx.strftime('%d/%m/%Y')}*\n\n"
                        f"💰 BTC: ${precio:,.2f} ({cambio:+.2f}% 24h)\n\n"
                        f"*SIMULADOR $1000:*\n"
                        f"Total: ${total:.2f}\n"
                        f"Ganancia NETA: ${ganancia_neta:+.2f} ({porc_neta:+.2f}%)\n"
                        f"Comisiones hoy: ${com_c+com_v:.2f}\n\n"
                        f"*TU INVERSIÓN REAL:*\n"
                        f"Compra: ${PRECIO_COMPRA_REAL:,.2f}\n"
                        f"Actual: ${precio:,.2f}\n"
                        f"Ganancia NETA real: ${ganancia_real:+.2f} ({porc_real:+.2f}%)\n\n"
                        f"Nos vemos mañana a las 10pm"
                    )
                    bot.send_message(int(CHAT_ID), reporte, parse_mode="Markdown")
                    ultimo_reporte_fecha = fecha_hoy
                    print(f"Reporte diario enviado {fecha_hoy}")
            time.sleep(60)
        except Exception as e:
...
@app.route('/')
def home():
    return "BTC Vicente Alert - Oportunidades + Reporte 10PM"
