...
def get_btc():
    # Intento 1 Coinbase (funciona en Render)
    try:
    ...
    while True:
        ...
        if CHAT_ID_SAVED and ENTRY:
            try:
                ...
                if p==0: continue
                if p <= ENTRY * 0.95:
                    send_msg(CHAT_ID_SAVED, "ALERTA SL -5% Compra " + str(round(ENTRY,2)) + " Ahora " + str(round(p,2)))
                if p >= ENTRY * 1.10:
                    send_msg(CHAT_ID_SAVED, "ALERTA TP +10% Compra " + str(round(ENTRY,2)) + " Ahora " + str(round(p,2)))
            except: pass
...
def home():
    p = get_btc()
    e = "Sin partida" if ENTRY is None else str(round(ENTRY,2))
    return VERSION + " BTC " + str(round(p,2)) + " ENTRY " + e, 200
...
def webhook():
    ...
    try:
        ...
        if "message" in data:
            ...
            btc = get_btc()
            if "/START" in txt or "ACT" in txt or "PRO" in txt or "GRAF" in txt:
                if ENTRY is None:
                    send_msg(chat_id, VERSION + "\nSin partida\nBTC REAL: " + str(round(btc,2)) + "\nDale COMPRAR")
                else:
                    sl = ENTRY * 0.95
                    tp = ENTRY * 1.10
                    gan = (btc/ENTRY-1)*100 if ENTRY else 0
                    send_msg(chat_id, VERSION + "\nENTRY: " + str(round(ENTRY,2)) + "\nAHORA: " + str(round(btc,2)) + "\nSL: " + str(round(sl,2)) + "\nTP: " + str(round(tp,2)) + "\nGan: " + str(round(gan,2)) + "%")
            elif "COMPRAR" in txt:
                ENTRY = btc
                sl = ENTRY * 0.95
                tp = ENTRY * 1.10
                send_msg(chat_id, "PARTIDA INICIADA\nCompra REAL: " + str(round(ENTRY,2)) + "\nSL: " + str(round(sl,2)) + "\nTP: " + str(round(tp,2)))
            elif "VENDER" in txt:
                if ENTRY is None:
