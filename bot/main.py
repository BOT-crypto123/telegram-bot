...
def wh():
 ...
 if 'GRAF' in t:
  ...
  if len(ema9)>0 and len(ema21)>0:
   ...
   if rsi<30: pred='SUBIDA FUERTE';score=82;senial='COMPRA FUERTE'
   if rsi>70: pred='BAJADA FUERTE';score=78;senial='VENTA FUERTE'
  if candles:
   i=0
   for c in candles:
    x=20+i*17
 ...
 if 'COMPRAR' in t:
  ...
  ENTS[SEL]={'entry':p,'chat':cid,'usd':m};save();send_text(cid,'COMPRADA '+SEL);return 'ok',200
 if 'VENDER' in t:
  if SEL in ENTS:
