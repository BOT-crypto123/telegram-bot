...
def do_buy(s, sym, price):
    ...
    s['h'][sym]={'a':(MONTO_MXN/17.5*0.998)/price,'e':price,'half':False,'orig_a':(MONTO_MXN/17.5*0.998)/price}
    s['b']-=MONTO_MXN
    return True, f"✅ COMPRADO {sym} ${price:.2f} con ${MONTO_MXN}"

def do_sell(s, sym, price, pct_sell=100):
    ...
    if pct_sell==50:
        ...
        recuperado=sell_amount*price*0.998*17.5
        ganancia_mxn = recuperado - (MONTO_MXN*0.5)
        s['b']+=recuperado
        s['h'][sym]['a']=amount*0.5
        s['h'][sym]['half']=True
        return True, f"⚠️ VENTA 50% {sym} {round(chg,2)}% ANTI-BARRIDO\nVendiste ${int(recuperado)} MXN te queda 50%", ganancia_mxn
    else:
        chg=(price/entry-1)*100
        recuperado=amount*price*0.998*17.5
        ganancia_mxn = recuperado - (MONTO_MXN*0.5) if s['h'][sym].get('half') else recuperado - MONTO_MXN
        s['b']+=recuperado
        s['total_trades']+=1
...
async def BRAIN_CHECK():
    ...
    for sym in ['BTC','ETH','SOL','XRP']:
        ...
        if sym in s['h']:
            ...
            is_half=s['h'][sym].get('half',False)
            if chg>=TAKE_PROFIT:
                if s.get('auto'):
                    ok,msg,gan=do_sell(s,sym,an['p'],100)
                    if ok:
                        S(s)
                        await ALERTAR_TODOS(f"🤖 AUTO VENTA 100% {sym} +{round(chg,2)}% ✅\n{msg}\nSaldo ${int(s['b'])}", sym)
                        s=L()
            elif chg<=STOP_LOSS_2:
                if s.get('auto'):
                    ok,msg,gan=do_sell(s,sym,an['p'],100)
                    if ok:
                        S(s)
                        await ALERTAR_TODOS(f"🤖 AUTO VENTA FINAL {sym} {round(chg,2)}% 🛑 -6%\n{msg}\nSaldo ${int(s['b'])}", sym)
                        s=L()
            elif chg<=STOP_LOSS_1 and not is_half:
                if s.get('auto'):
                    ok,msg,gan=do_sell(s,sym,an['p'],50)
                    if ok:
                        S(s)
                        await ALERTAR_TODOS(f"🤖 ANTI-BARRIDO {sym} {round(chg,2)}% -3% vendi 50%\n{msg}\nSaldo ${int(s['b'])}", sym)
                        s=L()
        else:
            if an['rsi']<35:
...
async def check():
    await BRAIN_CHECK()
    return {"ok":f"V951 TP {TAKE_PROFIT}% SL {STOP_LOSS_1}%/{STOP_LOSS_2}%"}
...
async def dash():
    ...
    pos_html="".join([f"<div class=card>{k} {'50%' if v.get('half') else '100%'} {round(({'BTC':an_btc,'ETH':an_eth,'SOL':an_sol,'XRP':an_xrp}[k]['p']/v['e']-1)*100,2)}%</div>" for k,v in s['h'].items()]) if s['h'] else "<div class=card>Sin posiciones</div>"
    html=f"""<html><head><meta name=viewport content="width=device-width,initial-scale=1"><script src="https://unpkg.com/lightweight-charts@4.1.3/dist/lightweight-charts.standalone.production.js"></script><style>body{{background:#0b0e14;color:#d1d4dc;font-family:sans-serif;margin:0}}.grid4{{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:10px}}.card{{background:#1a1f30;border:2px solid #2a3446;border-radius:16px;padding:12px;text-align:center;cursor:pointer}} #chart{{width:100%;height:460px}}</style></head><body><div style=padding:10px;text-align:center>V951 ANTI-CALZONES +2.5% / -3%50% / -6%100% Saldo ${int(s['b'])} <div class=grid4><div id=cBTC class=card onclick="loadSym('BTC')">BTC ${int(an_btc['p'])} RSI {int(an_btc['rsi'])}</div><div id=cETH class=card onclick="loadSym('ETH')">ETH ${int(an_eth['p'])} RSI {int(an_eth['rsi'])}</div><div id=cSOL class=card onclick="loadSym('SOL')">SOL ${int(an_sol['p'])} RSI {int(an_sol['rsi'])}</div><div id=cXRP class=card onclick="loadSym('XRP')">XRP {an_xrp['p']:.3f} RSI {int(an_xrp['rsi'])}</div></div><div id=chart></div><div class=grid4>{pos_html}</div></div><script>let current='BTC'; const DATA={{BTC:{{candles:{btc_c},e9:{btc_e9},e21:{btc_e21},e50:{btc_e50}}}},ETH:{{candles:{eth_c},e9:{eth_e9},e21:{eth_e21},e50:{eth_e50}}}},SOL:{{candles:{sol_c},e9:{sol_e9},e21:{sol_e21},e50:{sol_e50}}}},XRP:{{candles:{xrp_c},e9:{xrp_e9},e21:{xrp_e21},e50:{xrp_e50}}}}}; let chart,candleSeries,e9S,e21S,e50S; function createChart(){{const el=document.getElementById('chart');el.innerHTML='';chart=LightweightCharts.createChart(el,{{width:el.clientWidth,height:460,layout:{{background:{{type:'solid',color:'#0f1420'}},textColor:'#8a8d97'}}}});candleSeries=chart.addCandlestickSeries({{upColor:'#26a69a',downColor:'#ef5350'}});e9S=chart.addLineSeries({{color:'#ffcc00',lineWidth:1}});e21S=chart.addLineSeries({{color:'#ef5350',lineWidth:1}});e50S=chart.addLineSeries({{color:'#00ff88',lineWidth:1.2}});}} function toLine(c,a){{let o=[],s=c.length-a.length;for(let i=0;i<a.length;i++)o.push({{time:c[s+i].time,value:a[i]}});return o;}} function loadSym(s){{current=s;createChart();const d=DATA[s];candleSeries.setData(d.candles);e9S.setData(toLine(d.candles,d.e9));e21S.setData(toLine(d.candles,d.e21));e50S.setData(toLine(d.candles,d.e50));chart.timeScale().fitContent();}} createChart();loadSym('BTC');</script></body></html>"""
...
async def trade_api(action: str, sym: str):
    ...
    else: ok,msg,_=do_sell(s,sym.upper(),an['p'],100)
    if ok: S(s)
...
async def wh(req:Request):
    ...
    if 'callback_query' in q:
        ...
        if data=='auto_on': s['auto']=True; S(s); await G(cid,f'🚀 AUTO ON ANTI-CALZONES +2.5%/-3%/-6% Saldo ${int(s["b"])}', None)
        elif data=='auto_off': s['auto']=False; S(s); await G(cid,f'🔴 AUTO OFF Saldo ${int(s["b"])}', None)
        elif data.startswith('buy_'):
            sym=data.split('_')[1]; an=await AN(sym); ok,msg=do_buy(s,sym,an['p'])
            if ok: S(s); await G(cid,f'{msg}', sym)
        elif data.startswith('sell_'):
            sym=data.split('_')[1]; an=await AN(sym); ok,msg,_=do_sell(s,sym,an['p'],100)
            if ok: S(s); await G(cid,f'{msg}', sym)
        return {'ok':1}
    ...
    if 'RESET' in t: S({'b':1000,'h':{},'hs':[],'auto':False,'ganancia_total':0,'total_trades':0,'alert_users':s['alert_users']}); await G(cid,'♻️ RESET $1000 V951', None); return {'ok':1}
    if 'AUTO ON' in t: s['auto']=True; S(s); await G(cid,f'🚀 AUTO ON ANTI-CALZONES', None); return {'ok':1}
    if 'AUTO OFF' in t: s['auto']=False; S(s); await G(cid,f'🔴 AUTO OFF', None); return {'ok':1}
    if t in ['BTC','ETH','SOL','XRP']: an=await AN(t); await G(cid,f'*{t} ${an["p"]:.2f}* RSI {int(an["rsi"])}', t); return {'ok':1}
    await G(cid,f'V951 ANTI-CALZONES Saldo ${int(s["b"])}', None)
    return {'ok':1}
