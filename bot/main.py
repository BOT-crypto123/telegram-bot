...
def L():
    try:
        if os.path.exists(F):
            ...
            if d['b']==1000 and d['ganancia_total']==0 and len(d['h'])==0: d['b']=2000
            return d
    except: pass
...
async def SCORE(sym):
    ...
    if not cl or len(cl)<50: return {'p':pr,'score':0,'rsi':50,'tend':'LATERAL'}
    cl[-1]=pr
...
async def SEND(cid,txt, boton_dash=False, teclado=False):
    ...
    if boton_dash: payload['reply_markup']={"inline_keyboard": [[{"text":"📊 ABRIR DASHBOARD PUTERO","url":DASH_URL}]]}
    ...
    for sym in MONEDAS:
        ...
        if sym in s['h']:
            ...
            if chg>=3.5:
                ...
                for cid in s['alert_users']: await SEND(cid,f"🔥🔥 X2 {sym} +${monto*2}"); s=L()
            elif chg<=-6.0:
                g=SELL(s,sym,an['p'],100); S(s)
            ...
            if an['score']>=70 and btc_ok and s['b']>=monto and len(s['h'])<5:
                BUY(s,sym,an['p'],monto); S(s)
                for cid in s['alert_users']: await SEND(cid,f"🚀 ENTRA {sym} SCORE {an['score']} ${an['p']:.2f}", boton_dash=True); s=L()
...
async def reporte_diario_logic():
    ...
    gan=total-2000
    msg=f"📊 REPORTE 10PM V1004 $2000\n💰 Disp ${int(s['b'])} En pos ${int(total-s['b'])}\n💵 Total ${int(total)} Gan ${round(gan,2)} ({round((total/2000-1)*100,2)}%)\nHOY ${round(s['ganancia_hoy'],2)} Trades {s['trades_hoy']}"
    for cid in s.get('alert_users',[]): await SEND(cid,msg, boton_dash=True)
    return msg

@app.get('/check')
async def check(): await PUTERO(); return {"ok":"V1004 FIX"}
...
async def dash():
    ...
    def card(sym, full):
        an=scores[sym]
        col="#00ff88" if an['score']>=70 else "#ffcc33" if an['score']>=40 else "#ff3b4a"
        ...
        <div><div style="width:40px;height:40px;border-radius:50%;background:{col};color:#000;display:flex;align-items:center;justify-content:center;font-weight:900">{letter}</div>
        <div style="margin-top:6px"><b>{sym}</b><br><small style="opacity:.6">{full}</small></div></div>
    ...
    for k,v in s['h'].items():
        ...
        pos_html+=f"<div style='background:#151a2e;border-left:4px solid {col};padding:10px;border-radius:10px;margin-bottom:8px'><b>{k} x{v['niveles']}</b> inv ${v['invertido']} <span style='color:{col};float:right'>{chg:+.2f}%</span><br><small>Ent {v['e']:.2f} Ahora {pr:.2f}</small><br><b style='color:{col}'>${round(v['a']*pr*17.5,2)} MXN</b></div>"
    if not pos_html: pos_html="<div style='background:#151a2e;padding:16px;border-radius:12px;text-align:center;opacity:.5'>Sin posiciones abiertas<br><small>Esperando SCORE 70+</small></div>"
    ...
    if not hist: hist="<div style='opacity:.5;font-size:12px'>Historial aparecera aqui<br>Primer dia corriendo</div>"
 ...
 .header{{border:1.5px solid #00ffcc55;border-radius:20px;padding:12px;background:#0e1324;display:flex;justify-content:space-between;align-items:center}}
 ...
 .big{{font-size:26px;font-weight:900}}.g{{color:#00ff88}}.grid{{display:flex;flex-wrap:wrap}}
    ...
    <div class=top>
      ...
      <div class=box><small>📅 Hoy</small><div class=big style='color:{'#00ff88' if s['ganancia_hoy']>=0 else '#ff3b4a'}'>{'+' if s['ganancia_hoy']>=0 else ''}${round(s['ganancia_hoy'],0)}</div><small>• Hoy {time.strftime('%H:%M')}</small></div>
      <div class=box><small>🎯 Trades</small><div class=big>{s['total_trades']+len(s['h'])}</div><small>• {s['total_trades']} Win • {len(s['hs'])} Loss • {len(s['h'])} Open</small></div>
      <div class=box><small>🤖 Auto</small><div class=big style='color:#00ff88'>{'ON' if s['auto'] else 'OFF'}</div><small class=g>• Bot activo</small></div>
    </div>

    <div style='display:flex;justify-content:space-between;margin:10px 0'><b style='color:#5dfdcb'>MONEDAS</b><small style='opacity:.5'>SCORE 0-100</small></div>
    ...
    <div style='display:flex;gap:10px;margin-top:14px;flex-wrap:wrap'>
      ...
      <div style='flex:1 1 300px' class=box><b>Historial 7 dias</b> <span style='float:right'>🕒</span>
        <div style='height:60px;background:#0a1022;border-radius:8px;margin:10px 0;overflow:hidden'><svg width='100%' height='60'><polyline fill='none' stroke='#00ffcc' stroke-width='1.5' points='0,45 15,38 30,40 45,28 60,25 75,15 90,10 100,5'/></svg></div>
        {hist}
        <div style='margin-top:10px;border-top:1px solid #1a2340;padding-top:8px'><small style='opacity:.6'>Ultimos trades</small>{last}</div>
      </div>
    </div>
    ...
    <div style='display:flex;gap:8px;margin:15px 0'>
      ...
      <a href='/reporte_diario' style='flex:1;background:#00ff88;color:#000;text-align:center;padding:12px;border-radius:12px;text-decoration:none;font-weight:900'>📊 Reporte 10pm</a>
    </div>
    </body></html>
    """
...
async def wh(req:Request):
    ...
    if 'RESET' in t:
        ...
        await SEND(cid,"♻️ RESET $2000 PUTERO", teclado=True); return {'ok':1}
    if 'AUTO ON' in t: s['auto']=True; S(s); await SEND(cid,f"🔥 PUTERO $2000 ON Saldo ${int(s['b'])}", teclado=True); return {'ok':1}
    ...
    if 'DASHBOARD' in t or 'DASH' in t:
        total=s['b']
        for k,v in s['h'].items(): total+=v['a']*(await P(k))*17.5
        await SEND(cid,f"📊 V1004 PUTERO\nTotal ${int(total)} ({round((total/2000-1)*100,2)}%)\nHOY ${round(s['ganancia_hoy'],2)}", boton_dash=True); return {'ok':1}
    if t in MONEDAS:
    ...
    if 'PORTAFOLIO' in t:
        ...
        await SEND(cid,f"💼 PORTA Saldo ${int(s['b'])} Total ${int(total)}\nG/P ${round(total-2000,2)}\n{txt}", boton_dash=True); return {'ok':1}
    await SEND(cid,f"V1004 $2000 Saldo ${int(s['b'])} G/P ${round(s['ganancia_total'],2)}", teclado=True)
    return {'ok':1}
