 ...
 while True:
  ...
  try:
   for sy in ["BTC","ETH","SOL","XRP"]:
    cl=c(sy)
    if not cl: continue
    cs=[q[4] for q in cl]; rr=rsi(cs); pr=p(sy); e9=ema(cs,9); e21=ema(cs,21)
    if rr<30 and e9>e21 and not E.get(sy): E[sy]=pr; s2(CID,"AUTO COMPRO "+sy)
...
def w():
 ...
 if "SOL" in t: S="SOL"
 if "XRP" in t: S="XRP"
 if "AUTO" in t: ON=not ON; CID=cid; s2(cid,"AUTO ON" if ON else "AUTO OFF"); return "ok",200
 cl=c(S); cs=[q[4] for q in cl]
 if not cs: return "ok",200
 pr=p(S) or cs[-1]; rr=rsi(cs); pc=(pr/cs[-2]-1)*100 if len(cs)>1 else 0; e9=ema(cs,9); e21=ema(cs,21)
 sg="COMPRA FUERTE" if rr<30 else "VENTA FUERTE" if rr>70 else "ESPERA"
 pred,prob=("SUBIDA FUERTE",92) if e9>e21 and rr<30 else ("SUBIDA FUERTE",85) if e9>e21 and rr<50 else ("BAJADA FUERTE",82) if e9<e21 and rr>50 else ("SUBIDA",65) if e9>e21 else ("BAJADA",65) if e9<e21 else ("LATERAL",
