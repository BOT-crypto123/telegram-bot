...
for k,v in os.environ.items():
 ...
 if "UPSTASH" in k and "TOKEN" in k and "REDIS" in k and v!=BOT: TOK=v

KEY="btc-vicente-v36-1-final"
app=Flask(__name__)

@app.route("/")
def home(): return "V39.6.6 LIVE - FIX 404"
 ...
 try:
  ...
  if "message" in data and "text" in data["message"]:
   ...
   if "/start" in txt:
    ...
    f=f"V39.6.6 ON SL:{u.get('sl',-5)}% TP:+{u.get('tp',10)}%\nBTC {btc} ETH {eth} XRP {xrp}\nRSI XRP {rx}\nYA NO DA 404"
  ...
  if "callback_query" in data:
   ...
   if t is not None:
    rx=rsi("XRPUSDT"); f=f"V39.6.6 {'ON' if u.get('on') else 'OFF'} SL:{u.get('sl')}% TP:+{u.get('tp')}%\nBTC {btc} ETH {eth} XRP {xrp} RSI {rx}\n{t}"
    btn=[[{"text":"COMPRAR","callback_data":"COMPRAR"},{"text":"VENDER","callback_data":"VENDER"}],[{"text":"SL","callback_data":"SL"},{"text":"TP","callback_data":"TP"}],[{"text":"GRAF","callback_data":"GRAF"},{"text":"PRO","callback_data":"PRO"}],[{"text":"Apagar","callback_data":"APAGAR"}],[{"text":"ACT","callback_data":"ACT"}]]
    send(cid,f,btn)
   save(db)
