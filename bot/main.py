...
def getu(uid,d):
 ...
 if uid not in d["users"]:
  b,e,x,fx=market()
  d["users"][uid]={}
  d["users"][uid]["efectivo"]=0.0
  d["users"][uid]["btc"]=(333.33/fx)/b
  d["users"][uid]["eth"]=(333.33/fx)/e
  d["users"][uid]["xrp"]=(333.33/fx)/x
  d["users"][uid]["inicial"]=1000.0
  d["users"][uid]["stoploss"]=7.0
  d["users"][uid]["takeprofit"]=10.0
  d["users"][uid]["precio_compra"]={}
  d["users"][uid]["precio_compra"]["btc"]=b
  d["users"][uid]["precio_compra"]["eth"]=e
  d["users"][uid]["precio_compra"]["xrp"]=x
  d["users"][uid]["alertas"]=True
  d["users"][uid]["ultima_alerta"]={}
  save(d)
 u=d["users"][uid]
 if "alertas" not in u:
  u["alertas"]=True
...
def texto(u):
 ...
 s=f"V39.4 {al} SL:-{u['stoploss']:.0f}%"
 s+=f" TP:+{u['takeprofit']:.0f}%\n"
...
def kb_main(u):
 ...
 r=[]
 r.append([InlineKeyboardButton("COMPRAR",callback_data="menu_c")])
 r[0].append(InlineKeyboardButton("VENDER",callback_data="menu_v"))
 r.append([InlineKeyboardButton(f"SL -{u['stoploss']:.0f}%",callback_data="menu_sl")])
 r[1].append(InlineKeyboardButton(f"TP +{u['takeprofit']:.0f}%",callback_data="menu_tp"))
 r.append([InlineKeyboardButton("GRAFICA 7D",callback_data="grafica")])
 r[2].append(InlineKeyboardButton("PRO MAX",callback_data="pro"))
 r.append([InlineKeyboardButton(t,callback_data="toggle_alert")])
 r.append([InlineKeyboardButton("ACTUALIZAR",callback_data="act")])
 return InlineKeyboardMarkup(r)
def kb_pro():
 r=[]
 r.append([InlineKeyboardButton("BTC PRO",callback_data="pro_btc")])
 r[0].append(InlineKeyboardButton("ETH PRO",callback_data="pro_eth"))
 r.append([InlineKeyboardButton("XRP PRO",callback_data="pro_xrp")])
 r.append([InlineKeyboardButton("Volver",callback_data="act")])
 return InlineKeyboardMarkup(r)
def kb_sl():
 r=[]
 r.append([InlineKeyboardButton("-5%",callback_data="sl_5")])
 r[0].append(InlineKeyboardButton("-7%",callback_data="sl_7"))
 r[0].append(InlineKeyboardButton("-10%",callback_data="sl_10"))
 r.append([InlineKeyboardButton("Volver",callback_data="act")])
 return InlineKeyboardMarkup(r)
def kb_tp():
 r=[]
 r.append([InlineKeyboardButton("+10%",callback_data="tp_10")])
 r[0].append(InlineKeyboardButton("+15%",callback_data="tp_15"))
 r[0].append(InlineKeyboardButton("+20%",callback_data="tp_20"))
 r.append([InlineKeyboardButton("Volver",callback_data="act")])
 return InlineKeyboardMarkup(r)
def kb_c():
 r=[]
 r.append([InlineKeyboardButton("XRP $100",callback_data="c_xrp_100")])
 r[0].append(InlineKeyboardButton("BTC $100",callback_data="c_btc_100"))
 r.append([InlineKeyboardButton("ETH $100",callback_data="c_eth_100")])
 r.append([InlineKeyboardButton("Volver",callback_data="act")])
 return InlineKeyboardMarkup(r)
def kb_v():
 r=[]
 r.append([InlineKeyboardButton("Vender XRP",callback_data="v_xrp")])
 r[0].append(InlineKeyboardButton("Vender BTC",callback_data="v_btc"))
 r.append([InlineKeyboardButton("Vender ETH",callback_data="v_eth")])
 r.append([InlineKeyboardButton("Volver",callback_data="act")])
 return InlineKeyboardMarkup(r)
def get_rsi_for(moneda):
 try:
  url=f"https://api.exchange.coinbase.com/products/{moneda}/candles"
  url+=f"?granularity=3600"
  ...
  for i in range(14,len(deltas)):
   ...
   rs=avg_g/(avg_l if avg_l!=0 else 0.001)
...
def crear_grafica_pro(moneda):
 ...
 ax2.plot(times[-len(rsi):],rsi,color="#00aaff",linewidth=2)
   ...
   for uid,u in data["users"].items():
    try:
     ...
     for mon,rv in [("BTC",br),("ETH",er),("XRP",xr)]:
      ult=u["ultima_alerta"].get(mon,0)
      if time.time()-ult < 14400:
       continue
      if rv < 30:
       m=f"ALERTA COMPRA {mon} RSI {rv:.1f} SOBREVENDIDO"
       await app_bot.bot.send_message(chat_id=int(uid),text=m)
       u["ultima_alerta"][mon]=time.time()
       print(f"Alerta {mon} {uid}")
      elif rv > 70:
       m=f"ALERTA VENTA {mon} RSI {rv:.1f} SOBRECOMPRADO"
       await app_bot.bot.send_message(chat_id=int(uid),text=m)
       u["ultima_alerta"][mon]=time.time()
     data["users"][uid]=u
...
def start_bot_thread():
 ...
 async def bt(update,context):
  ...
  if k=="toggle_alert":
   ...
   return
  if k=="menu_c":
   await q.edit_message_text("Que compras?",reply_markup=kb_c())
   return
  if k=="menu_v":
   await q.edit_message_text("Que vendes?",reply_markup=kb_v())
   return
  if k=="menu_sl":
   await q.edit_message_text(f"SL -{u['stoploss']}%",reply_markup=kb_sl())
   return
  if k=="menu_tp":
   await q.edit_message_text(f"TP +{u['takeprofit']}%",reply_markup=kb_tp())
   return
  if k=="pro":
   await q.edit_message_text("PRO MAX:",reply_markup=kb_pro())
   return
  if k.startswith("sl_"):
  ...
  if k.startswith("c_"):
   mon=k.split("_")[1]
   if u["efectivo"]<100:
    await q.edit_message_text(f"Sin efectivo\n{texto(u)}",reply_markup=kb_main(u))
    return
   qty=(100/fx)/pr[mon]
   u[mon]+=qty
   u["efectivo"]-=100
  ...
  if k=="grafica":
   try:
    ...
    pa=crear_grafica_7d()
    f=open(pa,"rb")
    await q.message.reply_photo(photo=f,caption="7D %")
    f.close()
  ...
  if k.startswith("pro_"):
   try:
    ...
    f=open(pa,"rb")
    await q.message.reply_photo(photo=f,caption=f"{mon.upper()} RSI {rv:.1f}")
    f.close()
    await q.message.reply_text(texto(u),reply_markup=kb_main(u))
   except Exception as e:
    import traceback
    traceback.print_exc()
    await q.edit_message_text(f"Error PRO: {e}\n{texto(u)}",reply_markup=kb_main(u))
   return
